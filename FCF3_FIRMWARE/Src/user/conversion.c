...
static bool is_positive(int32_t value);
...
uint32_t integer_to_ascii(uint8_t *buffer, int32_t value)
{
    bool add_sign = false;
...
uint32_t float_to_ascii(uint8_t *buffer, float value, uint32_t accuracy)
{
    if (accuracy == 0)
    {
        accuracy = 1;
    ...
    uint32_t integer_part_lenght = integer_to_ascii(integer_part_buffer, integer_part);
    ...
    integer_to_ascii(fractional_part_buffer, fractional_part);
